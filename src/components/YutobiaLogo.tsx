...
export const YutobiaLogo: React.FC<LogoProps> = ({
  ...
  // Split "Youtopia" and "Multimedia" into individual characters for staggered animation
  const wordA = "Youtopia".split("");
  const wordB = "Multimedia".split("");

  return (
    <div className={`flex items-center gap-3 select-none ${className}`}>
      {/* ── Emblem with continuous slow rotation + glow pulse ─────────────── */}
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        {/* Ambient glow ring behind the mark */}
        <motion.div
          className="absolute inset-0 rounded-full"
          style={{
            background:
              "radial-gradient(circle, rgba(255,30,39,0.35) 0%, transparent 70%)",
          }}
          animate={{ opacity: [0.4, 0.85, 0.4], scale: [0.9, 1.1, 0.9] }}
          transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }}
        />

        {/* Slow-spinning SVG emblem */}
        <motion.svg
          width={size}
          height={size}
          viewBox="0 0 120 120"
      ...
      {/* ── Wordmark: "Youtopia Multimedia" with shimmer sweep ────────────── */}
      {showText && (
        <div className="flex flex-col leading-none gap-[3px]">
          {/* Row 1: "Youtopia" — each letter fades in staggered on mount, then steady */}
          <div className="relative overflow-hidden flex items-baseline">
            {/* The full wordmark text */}
            ...
            {/* Shimmer sweep overlay — slides across "Youtopia" continuously */}
            <motion.span
              aria-hidden
          ...
          {/* Row 2: "Multimedia" — slightly smaller, spaced mono feel */}
          <div className="relative overflow-hidden flex items-baseline">
            ...
            {/* Shimmer sweep for "Multimedia" — offset timing */}
            <motion.span
              aria-hidden
              className="pointer-events-none absolute inset-y-0 w-6"
              style={{
                background:
                  "linear-gradient(90deg, transparent 0%, rgba(255,30,39,0.3) 50%, transparent 100%)",
                mixBlendMode: "screen",
              }}
